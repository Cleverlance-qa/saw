...
public class MethodHelperSikuli extends MethodHelper {
    ...
    /**
     * Write text without defining element
     * @param keys - text which should be written
     */
    public void sendKeys(String keys) {
        logMessageToConsole("Typing " + keys);
        getSikuliScreen().type(keys);
    }

    /**
     * Write text without defining element without console output
     * @param keys - text which should be written
     */
    public void sendKeysHideOutput(String elementName, String keys) {
        logMessageToConsole("Typing text to " + elementName);
        dataReader.getSikuliElement(elementName).click();
        getSikuliScreen().type(keys);
    }
     ...
     * @param keys - text which should be written
     */
    public void sendKeysHideOutput(String keys) {
        logMessageToConsole("Typing text");
        getSikuliScreen().type(keys);
    }
    ...
    /**
     * Will wait until text appeart on screen
     * @param text - text to find
     * @param attempts - number of attempts
     * @return - text region coordinates
     */
    public Region waitForTextOnScreen(String text, int attempts) {
        for(int i = 0; i<=attempts; i++) {
            try {
                logMessageToConsole(String.format("Waiting for given text. Attempt number %s", i));
                final Region region = getSikuliScreen().findText(text);
                logMessageToConsole(text + " found");
                return region;
            } catch (FindFailed e) {
                LOGGER.log(Level.SEVERE, "Still waiting for text..", e);
            }
        }
        throw new TimeoutException(String.format("Text '%s' not exist!", text));
    }
